const reviewMap = {
    ...
    release: {
        version: "题库版本 v1.6.4",
        note: "本次更新：大数据原理与应用题库已开放，共 177 题；统一返回动态复习主页。"
    },
    subjects: [
        {
            id: "javaweb",
...
function renderSubjectNodes() {
    subjectLayer.innerHTML = reviewMap.subjects.map(subject => {
        const done = Math.min(answeredCount(subject.id), subject.questionCount);
        const progress = subject.questionCount ? Math.round((done / subject.questionCount) * 100) : 0;
        ...
        return `
            <button class="study-node subject-node ${reserved ? "is-reserved" : "is-ready"}"
                ...
                ${reserved ? 'aria-label="' + subject.name + '，题库预留"' : 'aria-label="' + subject.name + '，' + subject.questionCount + '题，已完成' + progress + '%"'}>
                <span>${subject.name}</span>
                <small>${reserved ? "题库预留" : `${subject.questionCount} 题 · 已刷 ${done}`}</small>
                ${reserved ? "" : `
    ...
    for (let pass = 0; pass < 4; pass += 1) {
        for (let i = 0; i < layoutState.length; i += 1) {
            ...
            for (let j = i + 1; j < layoutState.length; j += 1) {
                ...
                const bw = b.node.offsetWidth / 2 + padding;
                const bh = b.node.offsetHeight / 2 + padding;
                const dx = b.x - a.x;
                const dy = b.y - a.y;
